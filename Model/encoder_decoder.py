...
def cal_d(d, f1, f2, f3, f4, decoder, it=1, pred=None):
    for _ in range(it):
        pred_hat = decoder(f1, f2, f3, f4)
...
def get_r_adv_t_single(d, f1, f2, f3, f4, decoder, it=1, xi=1e-1, eps=10.0, pred=None):
    if pred is None:
        raise ValueError
    # f1, f2, f3, f4, f5 = f1.clone(), f2.clone(), f3.clone(), f4.clone(), f5.clone()
    if d.shape == f1.shape:
        d = cal_d(d, f1 + xi * d, f2, f3, f4, decoder, it, pred)
    elif d.shape == f2.shape:
        d = cal_d(d, f1, f2 + xi * d, f3, f4, decoder, it, pred)
    elif d.shape == f3.shape:
        d = cal_d(d, f1, f2, f3 + xi * d, f4, decoder, it, pred)
    elif d.shape == f4.shape:
        d = cal_d(d, f1, f2, f3, f4 + xi * d, decoder, it, pred)
    else:
        raise ValueError
...
def get_r_adv_t_mul(f1, f2, f3, f4, decoder, it=1, xi=1e-1, eps=10.0):
    decoder.eval()

    f1_de, f2_de, f3_de, f4_de = f1.detach(), f2.detach(), f3.detach(), f4.detach()

    with torch.no_grad():
        pred = F.softmax(decoder(f1_de, f2_de, f3_de, f4_de), dim=1)
    ...
    r_adv_1 = get_r_adv_t_single(d_1, f1_de, f2_de, f3_de, f4_de, decoder, it, xi, eps, pred)
    r_adv_2 = get_r_adv_t_single(d_2, f1_de, f2_de, f3_de, f4_de, decoder, it, xi, eps, pred)
    r_adv_3 = get_r_adv_t_single(d_3, f1_de, f2_de, f3_de, f4_de, decoder, it, xi, eps, pred)
    r_adv_4 = get_r_adv_t_single(d_4, f1_de, f2_de, f3_de, f4_de, decoder, it, xi, eps, pred)

    decoder.train()
    return r_adv_1, r_adv_2, r_adv_3, r_adv_4
...
class DecoderNetwork(nn.Module):
    ...
    def forward(self, f1, f2, f3, f4):
        # 解码器
        d3 = torch.cat([self.deconv_blk3(f4), f3], dim=1)
...
class VATDecoderNetwork(nn.Module):
    ...
    def forward(self, f1, f2, f3, f4, t_model=None):
        if t_model is not None:
            r_adv_1, r_adv_2, r_adv_3, r_adv_4 = get_r_adv_t_mul(f1, f2, f3, f4, t_model, it=1, xi=1e-6,
                                                                 eps=2.0)
            f1 = f1 + r_adv_1
            f2 = f2 + r_adv_2
            f3 = f3 + r_adv_3
