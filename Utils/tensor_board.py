...
class Tensorboard:
    ...
    def set_img_color(colors, background, img, pred):
        ...
        if len(pred[pred > len(colors)]) > 0:
            # original color
            if numpy.any(pred == background):
                img[numpy.where(pred == background)] = 0
            # change to pure white; class == 19
            else:
                img[numpy.where(pred > len(colors))] = 255
        return img / 255.0

    def update_wandb_pathology_image(self, images,
                                     teacher_prediction,
        ...
        for i in range(0, img_number):
            clean_pad = numpy.zeros_like(images[0][0].cpu().numpy())
            upload_prediction.append(self.set_img_color(self.get_class_colors(), 255, clean_pad,
                                                        predict_mask_hard[i].detach().cpu().numpy()))

        for i in range(0, img_number):
            clean_pad = numpy.zeros_like(images[0][0].cpu().numpy())
            upload_ground_truth.append(self.set_img_color(self.get_class_colors(), 255, clean_pad,
                                                          ground_truth[i].detach().cpu().numpy()))
        predict_mask_soft = predict_mask_soft.cpu().numpy()
        ...
        for i in range(0, img_number):
            ...
            upload_weight.append(plt.imread(os.path.join(confident_heat_map_path,
                                                         'step_{}/heatmap_{}.png'.format(str(self.current_step),
                                                                                         str(i)))))
